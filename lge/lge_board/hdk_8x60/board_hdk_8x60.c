...
#if defined(CONFIG_BATTERY_MSM8X60) || defined(CONFIG_LGE_PM)
...
/* jisun.shin@lge.com 2011.01.27
   IMX105 Main Camera GPIO on APQ Board for I-project*/
#ifdef CONFIG_LGE_CAMERA
static uint32_t camera_off_gpio_table[] = {
	GPIO_CFG(32, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /*CAM_MCLK*/
	GPIO_CFG(43, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), /*I2C_FLASH_SDA*/
	GPIO_CFG(44, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), /*I2C_FLASH_SCL*/
...
#endif
/* jisun.shin@lge.com 2011.01.27
   IMX105 Main Camera driver for I-project */
...
static struct i2c_board_info msm_camera_boardinfo[] __initdata = {
	/* jisun.shin@lge.com 2011.01.27
	   IMX105 Main Camera driver Board info for I-project */
	#ifdef CONFIG_LGE_SENSOR_IMX105
	{
...
int synaptics_t1320_power_on(int on, bool log_en)
{
	...
	vreg_l17 = regulator_get(NULL, "8058_l17");		/* +3V0_TOUCH */
	if (IS_ERR(vreg_l17)) {
		pr_err("%s: regulator get of 8058_l17 failed (%ld)\n", __func__, PTR_ERR(vreg_l17));
		rc = PTR_ERR(vreg_l17);
		return rc;
	}
	rc = regulator_set_voltage(vreg_l17, 3000000, 3000000);
...
{
	int rc = -EINVAL;
	printk(KERN_INFO "__debug___ %s: line: %d\n", __func__, __LINE__);
...
#define GPIO_GYRO_INT_1			46
#define GPIO_GYRO_INT_2			37
...
static struct lge_sensor_int_gpio lge_sensor_int_data[]=
{
...
#ifdef CONFIG_LGE_SENSOR_ACCELEROMETER
  {
      .num = GPIO_3AXIS_INT,
	  .name = "int_3axix",
	  .config = GPIO_CFG(GPIO_3AXIS_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
  },
#endif
#ifdef CONFIG_LGE_SENSOR_GYROSCOPE
  {
      .num = GPIO_GYRO_INT_2,
	  .name = "int_gyro",
	  .config = GPIO_CFG(GPIO_GYRO_INT_2, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
  },
#endif
#ifdef CONFIG_LGE_SENSOR_DCOMPASS
  {
      .num = GPIO_COMPASS_INT_2,
	  .name = "int_compass",
	  .config = GPIO_CFG(GPIO_COMPASS_INT_2, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
  },
#endif	
  {
      .num = 0,
...
static int lge_msm_sensor_init(void)
{
    int rc;
    int n;
...
static int sensor_common_power = false;
static int sensor_common_power_set(unsigned char onoff)
{
	printk(KERN_INFO "%s: line: %d\n", __func__, __LINE__);

	//power_set_for_8058_l23(onoff);

	if (onoff)
		sensor_common_power = true;	
	else
		sensor_common_power = false;	
...
static struct ami306_platform_data dcompss_pdata = {
	.pin_int1        = GPIO_COMPASS_INT_1,
	.pin_int2        = GPIO_COMPASS_INT_2,
	.power          = sensor_common_power_set,
	.accelerator_name = "k3dh_acc_misc",
	.fdata_sign_x = -1,
	.fdata_sign_y = -1,
	.fdata_sign_z = -1,
	.fdata_order0 = 1,
...
static int sensor_proximity_power = false;
static int sensor_proximity_power_set(unsigned char onoff)
{
	...
	if(onoff)
		sensor_proximity_power = true;
	else
		sensor_proximity_power = false;		
...
#if defined (CONFIG_LGE_SENSOR_DCOMPASS)|| defined (CONFIG_LGE_SENSOR_PROXIMITY) || defined (CONFIG_LGE_SENSOR_ACCELEROMETER)|| defined(CONFIG_LGE_SENSOR_GYROSCOPE)
#define APDS9900_ADDRESS 0x39
static struct i2c_board_info msm_i2c_gsbi12_info[] = {
	{
		I2C_BOARD_INFO("ami306", 0x0E),
		.irq =  MSM_GPIO_TO_INT(GPIO_COMPASS_INT_1),
	...
	},
	{
		...
		.irq =	-1/*MSM_GPIO_TO_INT(GPIO_GYRO_INT_1)*/,
...
static struct rpm_vreg_pdata rpm_vreg_init_pdata[RPM_VREG_ID_MAX] = {
	...
	RPM_VREG_INIT_LDO(PM8058_L1,  0, 1, 0, 1200000, 1200000, LDO300HMIN, 0), /* +1V2_8MCAM_DVDD */ // jisun.shin@lge.com
	...
	RPM_VREG_INIT_LDO(PM8058_L8,  0, 1, 0, 2800000, 2800000, LDO300HMIN, 0), /* +2V8_8MCAM_AF */ // jisun.shin@lge.com
	RPM_VREG_INIT_LDO(PM8058_L9,  0, 1, 0, 2800000, 2800000, LDO300HMIN, 0), /* +2V8_8MCAM_AVDD */ //jisun.shin@lge.com
	RPM_VREG_INIT_LDO(PM8058_L10, 0, 1, 0, 2600000, 2600000, LDO300HMIN, 0),
	RPM_VREG_INIT_LDO(PM8058_L11, 1, 1, 0, 3000000, 3000000, LDO150HMIN, 0),
	RPM_VREG_INIT_LDO(PM8058_L12, 1, 1, 0, 1800000, 1800000, LDO150HMIN, 0), /* +1V8_AUDIENCE_VDD */
	...
	RPM_VREG_INIT_LDO(PM8901_L1,  0, 1, 1, 3000000, 3000000, LDO300HMIN, 0), /* 3V0_MOTOR_VIB */ //sungwoo.cho@lge.com
	RPM_VREG_INIT_LDO(PM8901_L2,  0, 1, 0, 2850000, 3300000, LDO300HMIN, 0),
	/* 11.03.04 jaeseong.gim@lge.com modify LCD voltage */
	RPM_VREG_INIT_LDO(PM8901_L3,  0, 1, 0, 3000000, 3000000, LDO300HMIN, 0),
	...
	RPM_VREG_INIT_VS(PM8901_LVS1, 0, 1, 0,				 0),	/* +1V8_VTCAM_IOVDD*/ //jisun.shin@lge.com for Rev.B
...
static const unsigned int hdk_8x60_keymap[] = {
	KEY(0, 0, KEY_VOLUMEUP),
	KEY(0, 1, KEY_HOME),
	KEY(0, 2, KEY_VIDEO_PREV),
...
static struct matrix_keymap_data hdk_8x60_keymap_data = {
	.keymap_size	= ARRAY_SIZE(hdk_8x60_keymap),
	.keymap		= hdk_8x60_keymap,
};

static struct pmic8058_keypad_data hdk_8x60_keypad_data = {
	.input_name		= "hdk_8x60-keypad",
	.input_phys_device	= "hdk_8x60-keypad/input0",
	.num_rows		= 6,
	.num_cols		= 5,
	.rows_gpio_start	= 8,
	.cols_gpio_start	= 0,
	...
	.row_hold_ns            = 91500,
	.wakeup			= 1,
	.keymap_data		= &hdk_8x60_keymap_data,
};
...
static void __init msm8x60_board_init(struct msm_board_data *board_data)
{
	...
	pm8058_platform_data.sub_devices[PM8058_SUBDEV_KPD].
		platform_data = &hdk_8x60_keypad_data;
	pm8058_platform_data.sub_devices[PM8058_SUBDEV_KPD].data_size
		= sizeof(hdk_8x60_keypad_data);
...
#if defined(CONFIG_PM8058_CHARGER) || defined(CONFIG_LGE_PM)
