/*
 * NOTE: This example is works on x86 and powerpc.
 * Here's a sample kernel module showing the use of kprobes to dump a
 * stack trace and selected registers when do_fork() is called.
 *
 * For more information on theory of operation of kprobes, see
 * Documentation/kprobes.txt
 *
 * You will see the trace data in /var/log/messages and on the console
 * whenever do_fork() is invoked to create a new process.
 */

#include <linux/kernel.h>
#include <linux/module.h>
#include <linux/kprobes.h>

/* For each probe you need to allocate a kprobe structure */
static struct kprobe kp1 = {
	.symbol_name	= "pm_runtime_suspend", 
};
static struct kprobe kp2 = {
	.symbol_name	= "msm_pm_enter", 
};

/* kprobe pre_handler: called just before the probed instruction is executed */
static int handler_pre2(struct kprobe *p, struct pt_regs *regs)
{
	printk("\n### kprobe pre handler1 called !!\n");
	/* A dump_stack() here will give a stack backtrace */
	return 0;
}

/* kprobe post_handler: called after the probed instruction is executed */
static void handler_post2(struct kprobe *p, struct pt_regs *regs,
				unsigned long flags)
{
	printk("\n### kprobe post handler1 called !!\n");
}

/*
 * fault_handler: this is called if an exception is generated for any
 * instruction within the pre- or post-handler, or when Kprobes
 * single-steps the probed instruction.
 */
static int handler_fault2(struct kprobe *p, struct pt_regs *regs, int trapnr)
{
	printk(KERN_INFO "fault_handler: p->addr = 0x%p, trap #%dn",
		p->addr, trapnr);
	/* Return 0 because we don't handle the fault. */
	return 0;
}

/* kprobe pre_handler: called just before the probed instruction is executed */
static int handler_pre1(struct kprobe *p, struct pt_regs *regs)
{
	printk("\n### kprobe pre handler1 called !!\n");
	/* A dump_stack() here will give a stack backtrace */
	return 0;
}

/* kprobe post_handler: called after the probed instruction is executed */
static void handler_post1(struct kprobe *p, struct pt_regs *regs,
				unsigned long flags)
{
	printk("\n### kprobe post handler1 called !!\n");
}

/*
 * fault_handler: this is called if an exception is generated for any
 * instruction within the pre- or post-handler, or when Kprobes
 * single-steps the probed instruction.
 */
static int handler_fault1(struct kprobe *p, struct pt_regs *regs, int trapnr)
{
	printk(KERN_INFO "fault_handler: p->addr = 0x%p, trap #%dn",
		p->addr, trapnr);
	/* Return 0 because we don't handle the fault. */
	return 0;
}

static int __init kprobe_init(void)
{
	int ret;
	kp1.pre_handler = handler_pre1;
	kp1.post_handler = handler_post1;
	kp1.fault_handler = handler_fault1;

	ret = register_kprobe(&kp1);
	if (ret < 0) {
		printk(KERN_INFO "register_kprobe failed, returned %d\n", ret);
		return ret;
	}
	printk(KERN_INFO "Planted kprobe at %p\n", kp1.addr);

	kp2.pre_handler = handler_pre2;
	kp2.post_handler = handler_post2;
	kp2.fault_handler = handler_fault2;

	ret = register_kprobe(&kp2);
	if (ret < 0) {
		printk(KERN_INFO "register_kprobe failed, returned %d\n", ret);
		return ret;
	}
	printk(KERN_INFO "Planted kprobe at %p\n", kp2.addr);
	return 0;
}

static void __exit kprobe_exit(void)
{
	unregister_kprobe(&kp1);
	printk(KERN_INFO "kprobe at %p unregistered\n", kp1.addr);
	unregister_kprobe(&kp2);
	printk(KERN_INFO "kprobe at %p unregistered\n", kp2.addr);
}

module_init(kprobe_init)
module_exit(kprobe_exit)
MODULE_LICENSE("GPL");
