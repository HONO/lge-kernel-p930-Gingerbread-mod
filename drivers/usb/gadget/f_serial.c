...
//			__func__, no_tty_ports, no_sdio_ports, nr_ports);
	...
	if (no_tty_ports)
		ret = gserial_setup(c->cdev->gadget, no_tty_ports);	// happens
	if (no_sdio_ports)
		ret = gsdio_setup(c->cdev->gadget, no_sdio_ports);	// happens
	if (no_smd_ports)
...
static int gport_connect(struct f_gser *gser)
{
	...
	case USB_GADGET_FSERIAL_TRANSPORT_TTY:		// 0
		gserial_connect(&gser->port, port_num);
		break;
	case USB_GADGET_FSERIAL_TRANSPORT_SDIO:		// 1
		gsdio_connect(&gser->port, port_num);
		break;
	case USB_GADGET_FSERIAL_TRANSPORT_SMD:		// 2
...
// intf is interface number 0, 1 (f_serial has two interface)
